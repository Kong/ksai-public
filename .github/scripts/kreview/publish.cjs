...
const { RUN_SETTINGS_ARM_SHAPE, MODEL_TIERS, armLabel } = require('../lib/select-arm.cjs');
...
function evalRunRecord(env, { now = new Date() } = {}) {
  ...
  return {
    ...
    arm: {
      ...
      run_settings_arm: runSettingsArmRecord(env),
...
function runSettingsArmRecord(env) {
  const named = String(env.RUN_SETTINGS_ARM ?? '').trim();
