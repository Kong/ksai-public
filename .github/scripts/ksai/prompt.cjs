...
const PLAN_DOCUMENT_CONTRACT = Object.freeze([
  ...
  'A step is one `-` bullet starting in column 0. A second line under a step is refused rather than',
...
function renderPlanPrompt({
  ...
  const lines = [
    ...
    '    }',
    ...(writesDocument ? [
      '  The manifest carries no steps. They live in the document, so the plan a code owner edits is',
      '  the plan that runs.',
      ...
      '  waits for a code owner to review the commits and release the next one, so two to four phases is',
...
function renderRevisePrompt({
  ...
  const lines = [
    ...
    'document once a code owner approves it, and a plan the planner can tick is not a plan.',
    ...ONE_TURN,
    ...runContext({ budgetMinutes, channelNonce }),
    'These constraints apply to you and to every subagent you spawn.',
    '</system-instructions>',
