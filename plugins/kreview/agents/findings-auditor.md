...
12. **Comment quality uses comment evidence.** A `[delete]` or `[shrink]` finding about a changed source comment does not need a runtime failure. Verify the cited comment is added or edited, or changed code makes an adjacent claim stale. Keep it only when the exact code proves repetition, the prose contains no durable reason, or a useful reason is buried in narration. Remove taste, presumed AI authorship, required public API documentation, legal notices and tool directives. Keep `[delete]` and `[shrink]` findings Low. A false comment earns a higher tier only through a demonstrated misuse or failure.
