---
name: go-code-review
description: Adversarial Go code review for .go files, go.mod/go.sum changes, and PRs with Go code. Orchestrates an adversarial reviewer over the diff against 100+ common Go mistakes, then audits the findings with a findings auditor before reporting. Use when reviewing Go changes, a Go PR, or assessing Go implementation quality.
...
Orchestrate an adversarial Go review. The skill resolves the diff, reviews it under the [`go-code-reviewer`](reviewer.md) mandate beside this file, then runs the [`findings-auditor`](../../agents/findings-auditor.md) agent to red-team the findings before anything reaches the user. Two passes: one attacks the **code**, the next attacks the **findings**.
...
Take the diff from the path the invocation names, reading it once. Where it names none, determine the diff and the changed Go files. If nothing Go-related changed, say so and stop.
...
- The list of changed Go files, and the PR title and body when reviewing a PR.
...
- The name of the reviewer that produced them, `go-code-reviewer`, whose severity rubric the auditor enforces.
...
- 100+ Go mistakes catalog: [knowledge-base.md](knowledge-base.md)
